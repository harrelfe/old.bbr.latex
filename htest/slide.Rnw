...
<<bayesex4,h=6,w=8,cap="Posterior distributions for $\\mu$ and $\\sigma$ using a normal data model with weak priors (left panels), and convergence diagnostics for posterior sampling (right panels)",scap="Posterior distributions for $\\mu$ and $\\sigma$ using a normal model.">>=
plot(f)
@ 
...
\item The posterior mean, median, and mode of $\nu$
  (\Sexpr{fm(snu)}) provide evidence that the 
  data come from a distribution that is heavier tailed than the
  normal.  Posterior $P(\nu > 20) = \Sexpr{round(P(nu > 20), 2)}$
...
\item The traditional SD estimate, posterior median SD assuming normal
  ...
  respectively \Sexpr{fm(ssd)}.  The ordinary SD is giving too much
...
beta.post  <- 10 - 8 + beta
@ 
\item The solution is $\alpha=\beta=\Sexpr{round(alpha, 2)}$
\item With the data $s=8$ out of $n=10$ the posterior distribution is
  beta(\Sexpr{round(alpha.post, 2)}, \Sexpr{round(beta.post, 2)})

<<plotbetapost,bty='l',cap='Prior and posterior distribution for the unknown probability of heads.  The posterior is based on tossing 8 heads out of 10 tries.',scap='Prior and posterior distributions for unknown probability of heads'>>=
...
$p=0.5$ one needs $n=\Sexpr{nprec(0.05)}$.
...
<<pprecb,bty='l',h=4,w=6,cap='Half-widths of 0.95 credible intervals for $p$ using a flat prior'>>=
n <- seq(10, 400, by=5)
...
\item Example: \\
 Get a pooled estimate of $\sigma$ using $s$ above (\Sexpr{s}) \\
...
<<reraa,w=5,h=3,cap='Data for two-sample RCT for effect of caffeine on respiratory exchange ratio. Diamonds depict medians.',scap='Two-sample parallel group RCT'>>=
...
<<rerpass, w=5, h=3.75, bty='l', scap='Stratified ECDFs for checking $t$-test assumptions', cap='Stratified empirical cumulative distribution functions by treatment for checking all assumptions of the two-sample $t$-test.  ECDFs are inverse normal transformed.'>>=
...
<<bayesfit2e,mfrow=c(2,2),w=7,h=6>>=
...
of \Sexpr{nmin} subjects.  A pilot study with $n=20$ will achieve a
MMOE of \Sexpr{round(m[n == 20], 2)} in estimating $\sigma$.
