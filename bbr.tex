...
\newcommand{\blog}[1]{\marginpar{\href{http://fharrell.com/#1}{\rotatebox{270}{\fbox{\small\textcolor{blue}{blog}}}}}}
...
\def\originalBBR{0}  % originalBBR=1 if the original BBR; set to 0 otherwise
\ifnum\originalBBR=1
...
\def\changeHistoryShow{0}     % show change history if 1; set 0 otherwise
...
\url{vbiostatcourse.slack.com} channel \verb|#bbr|.  Anyone starting a
...
\def\basicStat{1}     % basicStat = 1 if only include basic statistics; set to 0 otherwise
\ifnum\basicStat=1
\input{R/slide}
\input{algebra}
\input{overview}
